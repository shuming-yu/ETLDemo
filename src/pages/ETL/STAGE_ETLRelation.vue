...
<template>
          ...
          <q-card-section class="q-pa-none">
            <q-table grid dense :rows="table.rows" :columns="table.columns" :filter="table.filter" :rows-per-page-options=[0]>
              <template v-slot:top-right>
                <q-btn @click="importDialogVisible = true" :label="t('Import')" rounded push color="positive" class="q-ml-xs" style="width: 100px" />
                  ...
                  <q-card class="no-shadow" bordered style="max-height: 115px;">
                    <q-card-section @click="navigateToEdit(props.row.Value, props.row.ETLVersion, props.row.Descr)" class="q-pb-none q-pt-sm cursor-pointer">
                      ...
                      <div class="row">
                        ...
                        <div class="col-2 flex flex-center">
                          <q-avatar size="lg" class="bordered-avatar">
                            {{ props.row.ETLVersion }}
                          </q-avatar>
                        </div>
                      </div>
                    </q-card-section>
                    ...
                    <q-card-actions class="row items-center justify-between q-pt-none">
                      <div class="col-6 q-gutter-sm q-px-sm" style="white-space: nowrap;">
                        <q-btn @click="navigateToEdit(props.row.Value, props.row.ETLVersion, props.row.Descr)" icon="edit" size="md" round flat dense class="bg-secondary text-white">
                          <q-tooltip class="bg-secondary text-body2" anchor="bottom middle" self="top middle">
                            <strong>{{ t("Edit") }}</strong>
                          </q-tooltip>
                        </q-btn>
                        <q-btn v-if="props.row.ETLVersion == PRD" @click="OpenExportDialog(props.row.Value)" icon="sym_o_file_copy" size="md" round flat dense class="bg-brown text-white">
                          <q-tooltip class="bg-brown text-body2" anchor="bottom middle" self="top middle">
                            <strong>{{ t("ExportToStage") }}</strong>
                          </q-tooltip>
                        </q-btn>
                        <q-btn @click="OpenDeleteDialog(props.row.Value)" icon="delete" size="md" round flat dense class="bg-negative text-white">
  ...
  <ETLDiagramImport v-model="importDialogVisible" @jsonFile="ImportDiagram" :isNewTopic="true" />
</template>

<script setup lang="ts">
import { type QTableProps } from "quasar";
import { ref, reactive, onMounted } from 'vue'
import { columnFormat, httpGetRequest, httpPostRequest, httpImportJsonFile, showSuccessNotify, showWarnNotify } from "@/utils/webUtil";
...
function getAllETLTopics() {
  let url = "ETLManager/ETLDiagram/GetAllETLTopics?version=" + STAGE;
  // let url = "ETLManager/ETLDiagram/GetAllETLTopics?version=";
...
function navigateToEdit(topic: string, version: string, descr: string) {
  ...
  flowDiagram.Version = STAGE;
  flowDiagram.Descr = descr;

  // 手動把 version 改為 STAGE
...
/** 跳出刪除Dialog */
function OpenDeleteDialog(topic: string) {
  let str = t("Topic") + ': ' + topic;
  let message = t("MsgConfirmDelete", [str]);
  ...
  Dialog.create({
    ...
    .onOk(() => {
      // DeleteTopicDiagram(topic);
    })
}

/** 刪除Topic Diagram */
function DeleteTopicDiagram(topic: string) {
  let url = "ETLManager/ETLDiagram/DeleteTopicDiagram?topic=" + topic;
  httpPostRequest(url, null)
    .then(res => {
      let message = `${t("Delete")}${t("Success")}`;
      showSuccessNotify(message);
...
function ImportDiagram(topic: string, database: string, file: File) {
  ...
  httpImportJsonFile<ETLFlow>(url, topic, database, file)
    .then(res => {
...
.bordered-avatar {
  border: 2px solid black; /* 設定外框為黑色 */
  border-radius: 50%;     /* 確保外框為圓形 */
  display: inline-flex;   /* 確保內容居中對齊 */
  align-items: center;
